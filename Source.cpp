...
public:
   ...
   void Clear(COLOR);
   bool GetKey(bool);
   void DrawString(int, int, std::string, COLOR color = White);
...
void Profiler::drawRam(int x, int y, uint16_t nAddr, int nRows, int nCols) {
   ...
   void Profiler::drawCpu(int x, int y) {
      ...
      DrawString(x + 64, y, "N", nes.cpu.status & arch6502::N ? Green : Red);
      DrawString(x + 80, y, "V", nes.cpu.status & arch6502::V ? Green : Red);
      DrawString(x + 96, y, "-", nes.cpu.status & arch6502::U ? Green : Red);
      DrawString(x + 112, y, "B", nes.cpu.status & arch6502::B ? Green : Red);
      DrawString(x + 128, y, "D", nes.cpu.status & arch6502::D ? Green : Red);
      DrawString(x + 144, y, "I", nes.cpu.status & arch6502::I ? Green : Red);
      DrawString(x + 160, y, "Z", nes.cpu.status & arch6502::Z ? Green : Red);
      DrawString(x + 172, y, "C", nes.cpu.status & arch6502::C ? Green : Red);
   ...
   void Profiler::drawCode(int x, int y, int nLines) {
      ...
      if(it != mapAsm.end()) {
         DrawString(x, nLinesY, it->second, Cyan);
